...
DEFAULT_OUT_DIR = Path("src/models/CSR-LANL/datasets_redteam_identity")
SPLITS = ("train", "val", "test")
GROUP_FOLD_SPLIT_MODES = {"groupkfold", "redteam_stratified_groupkfold"}
SUPPORTED_SPLIT_MODES = ["date", "random", "groupkfold", "redteam_stratified_groupkfold"]
...
IDENTITY_SKETCH_BUCKETS = 16
IDENTITY_SKETCH_PREFIXES = [
    "auth_dst_computer_id_bucket",
    "auth_src_dst_pair_id_bucket",
    "flow_dst_computer_id_bucket",
    "flow_dst_port_id_bucket",
    "flow_src_dst_pair_id_bucket",
    "flow_src_dst_port_pair_id_bucket",
    "dns_dst_computer_id_bucket",
    "dns_src_dst_pair_id_bucket",
]
IDENTITY_SKETCH_FEATURE_COLUMNS = [
    f"{prefix}_{bucket:02d}_count"
    for prefix in IDENTITY_SKETCH_PREFIXES
    for bucket in range(IDENTITY_SKETCH_BUCKETS)
]
IDENTITY_SUMMARY_FEATURE_COLUMNS = [
    "auth_src_dst_pair_unique_count",
    "flow_src_dst_pair_unique_count",
    "flow_src_dst_port_pair_unique_count",
    "dns_src_dst_pair_unique_count",
    "identity_unique_pair_count",
    "auth_dst_computer_new_count",
    "auth_dst_computer_new_ratio",
    "has_new_auth_dst_computer",
    "auth_src_dst_pair_new_count",
    "auth_src_dst_pair_new_ratio",
    "has_new_auth_src_dst_pair",
    "flow_dst_computer_new_count",
    "flow_dst_computer_new_ratio",
    "has_new_flow_dst_computer",
    "flow_dst_port_new_count",
    "flow_dst_port_new_ratio",
    "has_new_flow_dst_port",
    "flow_src_dst_pair_new_count",
    "flow_src_dst_pair_new_ratio",
    "has_new_flow_src_dst_pair",
    "flow_src_dst_port_pair_new_count",
    "flow_src_dst_port_pair_new_ratio",
    "has_new_flow_src_dst_port_pair",
    "dns_dst_computer_new_count",
    "dns_dst_computer_new_ratio",
    "has_new_dns_dst_computer",
    "dns_src_dst_pair_new_count",
    "dns_src_dst_pair_new_ratio",
...
FEATURE_COLUMNS = BASE_FEATURE_COLUMNS + IDENTITY_SKETCH_FEATURE_COLUMNS + IDENTITY_SUMMARY_FEATURE_COLUMNS
...
def identity_pair(src: str, dst: str) -> str:
    return f"{src}->{dst}"


def identity_port_pair(src: str, dst: str, port: str, protocol: str) -> str:
    return f"{src}->{dst}:{port}/{protocol}"


def stable_identity_bucket(value: str, buckets: int = IDENTITY_SKETCH_BUCKETS) -> int:
...
def add_identity_sketch(row: Dict[str, Any], prefix: str, values: Iterable[str]) -> None:
    for value in values:
        if not value:
            continue
        bucket = stable_identity_bucket(str(value))
        row[f"{prefix}_{bucket:02d}_count"] = float(row.get(f"{prefix}_{bucket:02d}_count", 0.0) + 1.0)


def add_novelty_features(row: Dict[str, Any], feature_prefix: str, values: set[str], seen_values: set[str]) -> bool:
    current_values = {str(value) for value in values if value}
    new_values = current_values - seen_values
    row[f"{feature_prefix}_new_count"] = float(len(new_values))
    row[f"{feature_prefix}_new_ratio"] = float(len(new_values) / len(current_values)) if current_values else 0.0
    row[f"has_new_{feature_prefix}"] = float(bool(new_values))
...
def empty_acc(window_start: int, window_seconds: int, entity: str) -> Dict[str, Any]:
    acc: Dict[str, Any] = {name: 0.0 for name in BASE_FEATURE_COLUMNS}
    acc.update(
        {
...
def update_auth(accumulators: MutableMapping[Tuple[int, str], Dict[str, Any]], chunk: pd.DataFrame, window_seconds: int) -> int:
    ...
    for (window_start, entity), group in chunk.groupby(["window_start", "entity"], sort=False):
        ...
        acc["_auth_pairs"].update(identity_pair(str(entity), v) for v in group["dst_computer_norm"] if v)
...
def update_flows(accumulators: MutableMapping[Tuple[int, str], Dict[str, Any]], chunk: pd.DataFrame, window_seconds: int) -> int:
    ...
    for (window_start, entity), group in chunk.groupby(["window_start", "entity"], sort=False):
        ...
        acc["_flow_pairs"].update(identity_pair(str(entity), v) for v in group["dst_computer_norm"] if v)
        for dst_computer, dst_port, protocol in zip(group["dst_computer_norm"], dst_ports, group["protocol_norm"]):
            if not dst_computer or pd.isna(dst_port):
                continue
            port = str(int(dst_port))
            acc["_flow_port_pairs"].add(identity_port_pair(str(entity), str(dst_computer), port, str(protocol)))
    return int(len(chunk))
...
def update_dns(accumulators: MutableMapping[Tuple[int, str], Dict[str, Any]], chunk: pd.DataFrame, window_seconds: int) -> int:
    ...
    for (window_start, entity), group in chunk.groupby(["window_start", "entity"], sort=False):
        ...
        acc["_dns_pairs"].update(identity_pair(str(entity), v) for v in group["dst_computer_norm"] if v)
    return int(len(chunk))
...
def finalize_windows(
    ...
    seen_auth_dst_computers_by_entity: Dict[str, set[str]] = defaultdict(set)
    seen_auth_pairs_by_entity: Dict[str, set[str]] = defaultdict(set)
    seen_flow_dst_computers_by_entity: Dict[str, set[str]] = defaultdict(set)
    seen_flow_dst_ports_by_entity: Dict[str, set[str]] = defaultdict(set)
    seen_flow_pairs_by_entity: Dict[str, set[str]] = defaultdict(set)
    seen_flow_port_pairs_by_entity: Dict[str, set[str]] = defaultdict(set)
    seen_dns_dst_computers_by_entity: Dict[str, set[str]] = defaultdict(set)
    seen_dns_pairs_by_entity: Dict[str, set[str]] = defaultdict(set)
    ...
    for key in sorted(accumulators):
        ...
        auth_dst_computers = set(acc["_auth_dst_computers"])
        auth_pairs = set(acc["_auth_pairs"])
        flow_dst_computers = set(acc["_flow_dst_computers"])
        flow_dst_ports = set(acc["_flow_dst_ports"])
        flow_pairs = set(acc["_flow_pairs"])
        flow_port_pairs = set(acc["_flow_port_pairs"])
        dns_dst_computers = set(acc["_dns_dst_computers"])
        dns_pairs = set(acc["_dns_pairs"])
        ...
        row["auth_src_dst_pair_unique_count"] = float(len(auth_pairs))
        row["flow_src_dst_pair_unique_count"] = float(len(flow_pairs))
        row["flow_src_dst_port_pair_unique_count"] = float(len(flow_port_pairs))
        row["dns_src_dst_pair_unique_count"] = float(len(dns_pairs))
        row["identity_unique_pair_count"] = float(len(auth_pairs | flow_pairs | dns_pairs | flow_port_pairs))
        add_identity_sketch(row, "auth_dst_computer_id_bucket", auth_dst_computers)
        add_identity_sketch(row, "auth_src_dst_pair_id_bucket", auth_pairs)
        add_identity_sketch(row, "flow_dst_computer_id_bucket", flow_dst_computers)
        add_identity_sketch(row, "flow_dst_port_id_bucket", flow_dst_ports)
        add_identity_sketch(row, "flow_src_dst_pair_id_bucket", flow_pairs)
        add_identity_sketch(row, "flow_src_dst_port_pair_id_bucket", flow_port_pairs)
        add_identity_sketch(row, "dns_dst_computer_id_bucket", dns_dst_computers)
        add_identity_sketch(row, "dns_src_dst_pair_id_bucket", dns_pairs)
        ...
        identity_novelty_flags = [
            add_novelty_features(row, "auth_dst_computer", auth_dst_computers, seen_auth_dst_computers_by_entity[entity]),
            add_novelty_features(row, "auth_src_dst_pair", auth_pairs, seen_auth_pairs_by_entity[entity]),
            add_novelty_features(row, "flow_dst_computer", flow_dst_computers, seen_flow_dst_computers_by_entity[entity]),
            add_novelty_features(row, "flow_dst_port", flow_dst_ports, seen_flow_dst_ports_by_entity[entity]),
            add_novelty_features(row, "flow_src_dst_pair", flow_pairs, seen_flow_pairs_by_entity[entity]),
            add_novelty_features(row, "flow_src_dst_port_pair", flow_port_pairs, seen_flow_port_pairs_by_entity[entity]),
            add_novelty_features(row, "dns_dst_computer", dns_dst_computers, seen_dns_dst_computers_by_entity[entity]),
            add_novelty_features(row, "dns_src_dst_pair", dns_pairs, seen_dns_pairs_by_entity[entity]),
        ]
        row["identity_novelty_signal_count"] = float(sum(1 for flag in identity_novelty_flags if flag))
...
def add_group_to_fold(fold: Dict[str, Any], entity: str, rows: int, attack_windows: int) -> None:
...
def redteam_stratified_groupkfold_split(df: pd.DataFrame, fold: int, n_folds: int) -> Dict[str, np.ndarray]:
    if n_folds < 3:
        raise SystemExit("redteam_stratified_groupkfold requires at least three folds")
    frame = df.loc[:, ["entity", "binary_target"]].copy()
    frame["is_attack"] = frame["binary_target"].astype(str).str.upper() != "BENIGN"
    summary = (
        frame.groupby("entity", sort=True)
        .agg(rows=("entity", "size"), attack_windows=("is_attack", "sum"))
        .reset_index()
    )
    if len(summary) < 3:
        raise SystemExit("Need at least three entities for redteam_stratified_groupkfold split")
    redteam_summary = summary[summary["attack_windows"] > 0].copy()
    if len(redteam_summary) < 3:
        raise SystemExit("Need at least three red-team entities for redteam_stratified_groupkfold split")
    ...
    target_min_redteam_entities = min(5, max(1, int(len(redteam_summary) // fold_count)))
    ...
    for index, row in enumerate(redteam_rows):
        if index < fold_count:
            add_group_to_fold(folds[index], str(row.entity), int(row.rows), int(row.attack_windows))
        else:
    ...
    while remaining_redteam_rows and any(item["redteam_entities"] < target_min_redteam_entities for item in folds):
        target_fold = min(
            range(fold_count),
        ...
        add_group_to_fold(folds[target_fold], str(row.entity), int(row.rows), int(row.attack_windows))
    ...
    for row in benign_summary.itertuples(index=False):
        ...
        add_group_to_fold(folds[target_fold], str(row.entity), int(row.rows), 0)

    mapped_fold = int(fold) % fold_count
    test_groups = folds[mapped_fold]["entities"]
    val_groups = folds[(mapped_fold + 1) % fold_count]["entities"]
    train_groups = set(summary["entity"].astype(str).tolist()) - test_groups - val_groups
    entity_values = df["entity"].astype(str)
    return {
        "train": df.index[entity_values.isin(train_groups)].to_numpy(dtype=int),
        "val": df.index[entity_values.isin(val_groups)].to_numpy(dtype=int),
        "test": df.index[entity_values.isin(test_groups)].to_numpy(dtype=int),
    }
...
def split_policy_payload(split_mode: str, split_indices: Dict[str, np.ndarray], df: pd.DataFrame, group_key: str) -> Dict[str, Any]:
    ...
    for split_name, indices in split_indices.items():
        ...
        payload["splits"][split_name] = {
            ...
            "redteam_entities": int(split_df.loc[split_df["redteam_exact"].astype(bool), "entity"].nunique()) if not split_df.empty else 0,
            "redteam_days": int((split_df.loc[split_df["redteam_exact"].astype(bool), "window_start"] // 86_400).nunique()) if not split_df.empty else 0,
...
def write_dataset_metadata(
    ...
    summary = {
        ...
        "identity_feature_profile": {
            "sketch_buckets": int(IDENTITY_SKETCH_BUCKETS),
            "sketch_prefixes": IDENTITY_SKETCH_PREFIXES,
            "sketch_feature_count": len(IDENTITY_SKETCH_FEATURE_COLUMNS),
            "summary_feature_count": len(IDENTITY_SUMMARY_FEATURE_COLUMNS),
            "policy": "Stable hashed buckets plus causal per-entity novelty features for destinations, ports, and source-destination pairs.",
        },
    }
    profile_config = {
        "dataset": args.dataset,
...
def build_split_indices(df: pd.DataFrame, args: argparse.Namespace, split_mode: str, fold: int | None = None) -> Dict[str, np.ndarray]:
    ...
    if split_mode == "redteam_stratified_groupkfold":
        return redteam_stratified_groupkfold_split(df, int(fold or 0), args.n_folds)
    ...
    for value in values:
        for part in str(value).split(","):
            ...
            if not candidate:
                continue
            if candidate == "all":
                modes.extend(SUPPORTED_SPLIT_MODES)
            elif candidate in SUPPORTED_SPLIT_MODES:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare CSR-LANL identity-enriched windowed Parquet datasets")
    ...
    print("== prepare_dataset_identity (CSR-LANL) ==")
