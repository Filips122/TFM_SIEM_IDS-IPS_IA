...
from models_torch import FTTransformer
...
def main():
    ...
    model_name = "offline_ML_binary_fttransformer"
    run_id = now_run_id()
    ...
    best_state, history, best_epoch = train_torch_classifier(
        model, tr_loader, va_loader, crit, opt, device, cfg, run_name="offline_ML_ftt", n_classes=len(le.classes_)
    )
    ...
    # Optional reports (day + groupkfold fold_4)
