...
def save_dataset_profile_ref(root: Path, split_mode: str, dataset: str, fold: Optional[int] = None, datasets_base: str | Path = "src/models/COWRIE_FULL/datasets") -> Dict[str, Any]:
    ref = dataset_profile_reference(datasets_base, split_mode, dataset, fold)
    save_json(root / "dataset_profile_ref.json", ref)
    return ref


def save_run_metadata(root: Path, model_name: str, split_mode: str, run_id: str, dataset: str = "COWRIE_FULL", datasets_base: str | Path = "src/models/COWRIE_FULL/datasets", run_config: Optional[Dict[str, Any]] = None) -> None:
...
def artifacts_root(model_name: str, split_mode: str, run_id: Optional[str] = None, dataset: str = "COWRIE_FULL", run_config: Optional[Dict[str, Any]] = None) -> Path:
    run_id = run_id or now_run_id()
    root = resolve_from_root(f"src/models/COWRIE_FULL/artifacts/{model_name}/{split_mode}/{run_id}")
    ensure_dir(root)
