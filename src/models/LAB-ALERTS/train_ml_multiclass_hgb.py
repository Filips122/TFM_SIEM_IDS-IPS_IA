...
def run_one(split_mode: str, fold: int | None, sample_frac: float | None, epochs: int, out_dir, dataset: str) -> dict:
    train, val, test = load_splits(split_mode=split_mode, dataset=dataset, pipeline="multiclass", fold=fold, sample_frac=sample_frac)
...
def main() -> None:
    ...
    root = artifacts_root("offline_LAB_ALERTS_multiclass_hgb", args.split_mode, now_run_id(), dataset=args.dataset, run_config={"sample_frac": args.sample_frac, "epochs": args.epochs})
